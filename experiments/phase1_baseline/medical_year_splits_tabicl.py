"""\
Phase 1 - Medical 年份切割基準線實驗（TabICL / TabPFN）
============================================================

對齊 `medical_year_splits_xgb.py` 與破產版 `bankruptcy_year_splits_tabicl.py`：

- 資料：UCI Diabetes 130-US Hospitals（1999–2008）
- 固定 Test = 2007-2008；訓練窗 1999–2006（8 年）
- 依 `experiments._shared.common_dataset.MEDICAL_YEAR_SPLITS` 共 7 組 Old/New 切割
- Validation：訓練段內依 **year 逐年**各抽約 20%（`_split_fit_val_by_year`）
- Scaler 僅在 train fold 上 fit；採樣僅施加於 train fold
- 閾值：validation 上 F1 grid search（0.05–0.95，步長 0.01）
- 策略：Old / New / Retrain（Retrain 僅在第一個 split 跑一次）
- 採樣：none / undersampling / oversampling / hybrid

輸出：
  - results/phase1_baseline/tabicl/<tag>/medical_year_splits_tabicl_raw.csv
  - med_tabicl_table_{metric}_{old|new|retrain}.csv
  - med_tabicl_compact_summary.csv
  - med_tabicl_compact_{AUC|F1|Recall}_only.csv

安裝依賴：pip install tabpfn（或 tabicl）
"""
...
CHECKPOINT_NAME = "medical_year_splits_tabicl_checkpoint.json"
PARTIAL_RAW_NAME = "medical_year_splits_tabicl_raw.partial.csv"
...
def format_tables(df_raw, logger, output_dir: Path):
    split_yr = {
        label: (old_end - BASE_YEAR + 1, TRAIN_END_YEAR - old_end) for label, old_end in MEDICAL_YEAR_SPLITS
    }
    ...
    for metric in METRICS:
        ...
        out = output_dir / f"med_tabicl_table_{metric}_old.csv"
        ...
        out = output_dir / f"med_tabicl_table_{metric}_retrain.csv"
        ...
        out = output_dir / f"med_tabicl_table_{metric}_new.csv"
...
def export_compact_report(df_raw: pd.DataFrame, logger, output_dir: Path) -> None:
    ...
    path_all = output_dir / "med_tabicl_compact_summary.csv"
    ...
    for m in COMPACT_ONLY_METRICS:
        ...
        path_m = output_dir / f"med_tabicl_compact_{m}_only.csv"
...
def _save_raw_snapshot(rows: list[dict], output_dir: Path, logger) -> None:
    ...
    if not rows:
        return
    raw_path = output_dir / "medical_year_splits_tabicl_raw.csv"
...
def _load_progress(output_dir: Path, logger):
    ...
    partial_raw = output_dir / PARTIAL_RAW_NAME
...
def _run_one_output_dir(
    ...
    *,
    ...
    for label, old_end_year in MEDICAL_YEAR_SPLITS:
        ...
        logger.info("=" * 60)
        try:
            X_old, y_old, X_new, y_new, X_test, y_test, year_old, year_new, year_test = get_medical_year_split(
...
def main():
    parser = argparse.ArgumentParser(description="Phase1 medical year splits — TabICL (TabPFN) baseline")
    parser.add_argument(
        "--device",
    ...
    logger = get_logger("MED_YearSplits_TabICL", console=True, file=True)
    set_seed(42)
