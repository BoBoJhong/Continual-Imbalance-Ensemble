"""
Phase 1 - Medical 年份切割基準線實驗（MLP）
=======================================================
架構：sklearn MLPClassifier (128→64→32→1，ReLU，Adam)
不平衡處理：ImbalanceSampler（與 XGBoost 版完全一致）

固定 Test = 2007-2008，7 組切割 × 3 策略 × 4 採樣 = 84 rows

最終輸出：
  - results/phase1_baseline/medical_year_splits_mlp_raw.csv
  - results/phase1_baseline/med_mlp_table_{metric}_{old|oldnew|new}.csv
"""
import sys
...
def format_tables(df_raw, logger):
    split_yr = {label: (old_end - BASE_YEAR + 1, 2006 - old_end) for label, old_end in MEDICAL_YEAR_SPLITS}
    df_raw = df_raw.copy()
    ...
    for metric in METRICS:
        for method, suffix, idx_fn in [
            ...
            out = OUTPUT_DIR / f"med_mlp_table_{metric}_{suffix}.csv"
            pivot.to_csv(out, float_format="%.4f")
            logger.info(f"  Saved -> {out.name}")


def main():
    logger = get_logger("Medical_YearSplits_MLP", console=True, file=True)
    set_seed(42)
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    all_rows = []

    for label, old_end_year in MEDICAL_YEAR_SPLITS:
        logger.info(f"\n{'='*60}\nSplit: {label}  (Old<={old_end_year}, New={old_end_year+1}-2006, Test=2007-2008)\n{'='*60}")
        try:
            X_old, y_old, X_new, y_new, X_test, y_test = get_medical_year_split(logger, old_end_year=old_end_year)
    ...
    raw_path = OUTPUT_DIR / "medical_year_splits_mlp_raw.csv"
