"""
Phase 1 - Bankruptcy 年份切割基準線實驗（MLP）
=======================================================
架構：sklearn MLPClassifier (128→64→32→1，ReLU，Adam)
不平衡處理：ImbalanceSampler（採樣後餵入 MLP，與 XGBoost 版策略完全一致）

固定 Test = 2015-2018，7 組切割 × 3 策略 × 4 採樣 = 84 rows

最終輸出：
  - results/phase1_baseline/bankruptcy_year_splits_mlp_raw.csv
  - results/phase1_baseline/bk_mlp_table_{metric}_{old|oldnew|new}.csv
"""
import sys
...
def format_tables(df_raw, logger):
    ...
    for metric in METRICS:
        for method, suffix, idx_fn in [
            ...
            out = OUTPUT_DIR / f"bk_mlp_table_{metric}_{suffix}.csv"
            pivot.to_csv(out, float_format="%.4f")
            logger.info(f"  Saved -> {out.name}")


def main():
    logger = get_logger("BK_YearSplits_MLP", console=True, file=True)
    set_seed(42)
    ...
    raw_path = OUTPUT_DIR / "bankruptcy_year_splits_mlp_raw.csv"
