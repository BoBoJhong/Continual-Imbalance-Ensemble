"""
Phase 1 - Bankruptcy 年份切割基準線實驗（LSTM）
=======================================================
架構：PyTorch LSTM（tabular 特徵視為序列）
不平衡處理：ImbalanceSampler（採樣後餵入 LSTM，與 XGBoost 版策略一致）

固定 Test = 2015-2018，7 組切割 × 3 策略 × 4 採樣 = 84 rows

安裝依賴：pip install torch

最終輸出：
  - results/phase1_baseline/lstm/bankruptcy_year_splits_lstm_raw.csv
  - results/phase1_baseline/lstm/bk_lstm_table_{metric}_{old|oldnew|new}.csv
"""
import sys
...
OUTPUT_DIR          = project_root / "results" / "phase1_baseline" / "lstm"
...
def _train_eval(X_train, y_train, X_test, y_test, sampler, strategy, tag, logger):
    ...
    model = LSTMWrapper(name=f"{tag}_{strategy}")
    model.fit(X_r, y_r)
...
def format_tables(df_raw, logger):
    ...
    for metric in METRICS:
        for method, suffix, idx_fn in [
            ...
            out = OUTPUT_DIR / f"bk_lstm_table_{metric}_{suffix}.csv"
            pivot.to_csv(out, float_format="%.4f")
            logger.info(f"  Saved -> {out.name}")


def main():
    logger = get_logger("BK_YearSplits_LSTM", console=True, file=True)
    set_seed(42)
    ...
    raw_path = OUTPUT_DIR / "bankruptcy_year_splits_lstm_raw.csv"
