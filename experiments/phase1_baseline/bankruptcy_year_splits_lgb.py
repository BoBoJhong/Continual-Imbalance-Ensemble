"""\
Phase 1 - Bankruptcy 年份切割基準線實驗（LightGBM + scale_pos_weight）
===============================================================
與 `bankruptcy_year_splits_xgb.py` 同協定：
  - 固定 Test = 2015-2018；訓練窗 1999–2014（16 年）
  - 依 `experiments._shared.common_bankruptcy.YEAR_SPLITS` 逐年滑動 old_end，共 15 組 Old/New 切割
  - Validation：訓練段內依 fyear 逐年各抽約 20% 作 validation，合併為校準集

訓練策略：
  - Old      : 只用歷史 (Old) 資料訓練
  - New      : 只用新營運 (New) 資料訓練
  - Retrain  : 歷史 + 新營運全量合併後訓練（整個實驗只跑一次）

採樣策略：none / undersampling / oversampling / hybrid

不平衡處理：LGBM 使用 `scale_pos_weight = n_negative / n_positive`（由採樣後的訓練資料計算）。
...
用法：
  python experiments/phase1_baseline/bankruptcy_year_splits_lgb.py --tuning tuned --tune-n-iter 48 --output-tag bk_lgb_tuned48_20260421
  python experiments/phase1_baseline/bankruptcy_year_splits_lgb.py --tuning both  --tune-n-iter 48 --output-tag bk_lgb_both_20260421
"""
...
TRAIN_END_YEAR = 2014
TOTAL_TRAIN_YEARS = TRAIN_END_YEAR - TRAIN_START_YEAR + 1
...
def _split_fit_val_by_year(
    ...
    *,
    ...
    """依 year 分層切 validation：每個年分抽約 val_ratio 當 val。"""
...
def format_tables(df_raw: pd.DataFrame, logger, output_dir: Path) -> None:
    ...
    split_yr = {label: (old_end - TRAIN_START_YEAR + 1, TRAIN_END_YEAR - old_end) for label, old_end in YEAR_SPLITS}
    ...
    for metric in METRICS:
        ...
        out = output_dir / f"bk_lgb_table_{metric}_old.csv"
        pivot_old.to_csv(out, float_format="%.4f")
        logger.info(f"  Saved -> {out.name}")

        # Retrain（full 16yr 單列）
        ...
        out = output_dir / f"bk_lgb_table_{metric}_retrain.csv"
        ...
        out = output_dir / f"bk_lgb_table_{metric}_new.csv"
...
def export_compact_report(df_raw: pd.DataFrame, logger, output_dir: Path) -> None:
    ...
    if rows:
        out = output_dir / "bk_lgb_compact_summary.csv"
    ...
    for m in COMPACT_ONLY_METRICS:
        ...
        out = output_dir / f"bk_lgb_compact_{m}_only.csv"
...
def _run_one_output_dir(
    ...
    *,
    ...
    for label, old_end_year in YEAR_SPLITS:
        ...
        logger.info(f"\n{'='*60}")
        logger.info(
            f"Split: {label}  (Old<={old_end_year}, New={old_end_year + 1}-{TRAIN_END_YEAR}, Test=2015-2018)"
        )
        logger.info("=" * 60)
    ...
    if not all_rows:
        logger.error("無任何結果，請確認 US 破產資料檔存在（american_bankruptcy_dataset.csv）。")
        return

    df_raw = pd.DataFrame(all_rows)
    raw_path = output_dir / "bankruptcy_year_splits_lgb_raw.csv"
    ...
    if use_tuning:
        export_tuning_log(df_raw, output_dir / "bk_lgb_tuning_log.csv", logger)
...
def main() -> None:
    p = argparse.ArgumentParser(description="Phase1 bankruptcy year splits — LightGBM baseline (scale_pos_weight)")
    p.add_argument(
        "--tuning",
        type=str,
        default="default",
    ...
    logger = get_logger("Bankruptcy_YearSplits_LGB", console=True, file=True)
    set_seed(42)
