"""
Phase 1 - Stock 年份切割基準線實驗（MLP）
=======================================================
架構：sklearn MLPClassifier (128→64→32→1，ReLU，Adam)
不平衡處理：ImbalanceSampler（與 XGBoost 版完全一致）

固定 Test = 2017-2020，訓練窗口 2001-2016（16 年）
7 組對稱切割（2+14…14+2）× 3 策略 × 4 採樣 = 84 rows

閾值：class-prior threshold（訓練集正類比例）
      股票崩盤率僅 ~7%，固定 0.5 會導致 F1=0

最終輸出：
  - results/phase1_baseline/stock_year_splits_mlp_raw.csv
  - results/phase1_baseline/stk_mlp_table_{metric}_{old|oldnew|new}.csv
"""
import sys
...
from src.models import MLPWrapper
...
def format_tables(df_raw, logger):
    split_yr = {label: (old_end - STOCK_BASE_YEAR + 1, 2016 - old_end) for label, old_end in STOCK_YEAR_SPLITS}
    df_raw = df_raw.copy()
    ...
    for metric in METRICS:
        for method, suffix, idx_fn in [
            ...
            out = OUTPUT_DIR / f"stk_mlp_table_{metric}_{suffix}.csv"
            pivot.to_csv(out, float_format="%.4f")
            logger.info(f"  Saved -> {out.name}")


def main():
    logger = get_logger("Stock_YearSplits_MLP", console=True, file=True)
    set_seed(42)
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    all_rows = []

    for label, old_end_year in STOCK_YEAR_SPLITS:
        logger.info(f"\n{'='*60}\nSplit: {label}  (Old=2001-{old_end_year}, New={old_end_year+1}-2016, Test=2017-2020)\n{'='*60}")
        try:
            X_old, y_old, X_new, y_new, X_test, y_test = get_stock_year_split(logger, old_end_year=old_end_year, ticker=TICKER)
    ...
    raw_path = OUTPUT_DIR / "stock_year_splits_mlp_raw.csv"
    df_raw.to_csv(raw_path, index=False, float_format="%.6f")
    logger.info(f"\n原始結果已儲存 -> {raw_path.name}  ({len(df_raw)} rows)")
    logger.info("\n產出指標 pivot 表格...")
    format_tables(df_raw, logger)
    logger.info("\n=== 完成 ===")
    summary = df_raw.groupby(["method", "sampling"])["AUC"].mean().unstack("sampling").reindex(["Old", "Old+New", "New"])
    logger.info("\nAUC 摘要（MLP，跨7組切割，2+14…14+2）:\n" + summary.to_string())
