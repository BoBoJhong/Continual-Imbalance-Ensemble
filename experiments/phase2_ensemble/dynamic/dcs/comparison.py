"""
Phase 2 — DCS Comparison: OLA / LCA / OLA_TW / LCA_TW
...
def _run_and_save(name, X_h, y_h, X_n, y_n, X_t, y_t, logger):
    ...
    df.to_csv(OUTPUT_DIR / f"{name}_dcs_comparison.csv")


def main():
    logger = get_logger("Phase3_DCS", console=True, file=True)
    set_seed(42)
    ...
    logger.info("\nPhase 2 DCS 完成。results/phase2_ensemble/dynamic/dcs/")
