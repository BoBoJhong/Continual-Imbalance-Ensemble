"""Phase 2 — XGB Stock(spx)：動態 DCS，年份切割（單一 scaler + 6 XGB）。輸出：dynamic/dcs/"""
...
def main():
    ticker = "spx"
    logger = get_logger("XGB_Stock_DCS_YearSplits", console=True, file=True)
    set_seed(42)
    ...
    path = out / f"xgb_oldnew_ensemble_dcs_by_sampling_raw_stock_{ticker}.csv"
