"""
Phase 2 — DES Advanced: 時間加權 / 少數類加權 / Combined 進階 DES
涵蓋全部資料集 (Bankruptcy / Stock / Medical)。
"""
import sys
from pathlib import Path
import pandas as pd

project_root = Path(__file__).resolve().parent.parent.parent.parent.parent
sys.path.insert(0, str(project_root))

from src.utils import set_seed, get_logger
from src.evaluation import results_to_dataframe
from experiments._shared.common_bankruptcy import get_bankruptcy_splits
from experiments._shared.common_dataset import get_splits
from experiments._shared.common_des import run_des
from experiments._shared.common_des_advanced import run_des_advanced

SPLIT_MODE = "block_cv"
OUTPUT_DIR = project_root / "results" / "phase2_ensemble" / "dynamic" / "des"


def run_experiment(X_h, y_h, X_n, y_n, X_t, y_t, logger):
    results = {
        "DES_baseline":          run_des(X_h, y_h, X_n, y_n, X_t, y_t, logger, k=7),
        "DES_time_weighted":     run_des_advanced(X_h, y_h, X_n, y_n, X_t, y_t, logger, k=7, time_weight_new=2.0, minority_weight=1.0),
        "DES_minority_weighted": run_des_advanced(X_h, y_h, X_n, y_n, X_t, y_t, logger, k=7, time_weight_new=1.0, minority_weight=2.0),
        "DES_combined":          run_des_advanced(X_h, y_h, X_n, y_n, X_t, y_t, logger, k=7, time_weight_new=2.0, minority_weight=2.0),
    }
    for name, m in results.items():
        logger.info(f"  {name:28s}: AUC={m['AUC']:.4f}  F1={m['F1']:.4f}")
    return results_to_dataframe(results)


def _run_and_save(name, X_h, y_h, X_n, y_n, X_t, y_t, logger):
    df = run_experiment(X_h, y_h, X_n, y_n, X_t, y_t, logger)
    df.to_csv(OUTPUT_DIR / f"{name}_des_advanced.csv")


def main():
    logger = get_logger("Phase3_DES_Advanced", console=True, file=True)
    set_seed(42)
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

    logger.info("=" * 70 + "\nBankruptcy  (DES Advanced)\n" + "=" * 70)
    _run_and_save("bankruptcy", *get_bankruptcy_splits(logger, split_mode=SPLIT_MODE), logger)

    for ds in ["stock_spx", "stock_dji", "stock_ndx"]:
        logger.info(f"{'=' * 70}\n{ds.upper()}  (DES Advanced)\n{'=' * 70}")
        try:
            _run_and_save(ds, *get_splits(ds, logger, split_mode=SPLIT_MODE), logger)
        except Exception as e:
            logger.error(f"[ERROR] {ds}: {e}")

    logger.info("=" * 70 + "\nMedical  (DES Advanced)\n" + "=" * 70)
    try:
        _run_and_save("medical", *get_splits("medical", logger, split_mode=SPLIT_MODE), logger)
    except Exception as e:
        logger.error(f"[ERROR] medical: {e}")

    logger.info("\nPhase 2 DES Advanced 完成。results/phase2_ensemble/dynamic/des/")


if __name__ == "__main__":
    main()
