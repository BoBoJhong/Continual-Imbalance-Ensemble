"""Phase 2 — XGB Medical：靜態集成，年份切割。輸出：results/phase2_ensemble/static/"""
...
from experiments.phase2_ensemble.xgb_year_split_shared import iter_medical_year_splits
...
def main():
    logger = get_logger("XGB_Medical_Static_YearSplits", console=True, file=True)
    set_seed(42)

    out = project_root / "results" / "phase2_ensemble" / "static"
    out.mkdir(parents=True, exist_ok=True)

    static_rows, _ = iter_medical_year_splits(logger)
    ...
    logger.info("\n完成。static/ xgb_oldnew_ensemble_static_*_medical.csv")
