"""
Phase 3 — Study II: 股票資料集 (Stock - SPX) 的全集成與特徵選擇實驗
=============================================================================
資料集：S&P 500 (2001-2020)
測試 no_fs 與 mutual_info_r80 在股票市場崩盤預測上的集成表現。
"""
...
from experiments._shared.common_dataset import STOCK_YEAR_SPLITS, get_stock_year_split
...
OUTPUT_DIR = results_method_dir(project_root, "mutual_info")
...
def main():
    logger = get_logger("Phase3_FS_Full_Stock", console=True, file=True)
    set_seed(42)
    ...
    for split_idx, (label, old_end_year) in enumerate(STOCK_YEAR_SPLITS, 1):
        logger.info(f"\n[{split_idx}/{len(STOCK_YEAR_SPLITS)}] Processing {label}...")
        
        try:
            X_old_raw, y_old, X_new_raw, y_new, X_test_raw, y_test, year_old, year_new, _ = \
                get_stock_year_split(logger, old_end_year=old_end_year, ticker="spx", return_years=True)
        ...
        for fs_tag, fs_method, fs_ratio in FS_CONFIGS:
            ...
            for s in SAMPLING_STRATEGIES:
                ...
                m = XGBoostWrapper(name=f"old_stock_{label}_{fs_tag}_{s}")
                m.fit(X_r, y_r)
                ...
                m = XGBoostWrapper(name=f"new_stock_{label}_{fs_tag}_{s}")
                m.fit(X_r, y_r)
    ...
    # Save results
    pd.DataFrame(static_all).to_csv(OUTPUT_DIR / "xgb_stock_fs_full_static.csv", index=False)
    pd.DataFrame(des_all).to_csv(OUTPUT_DIR / "xgb_stock_fs_full_des.csv", index=False)
    logger.info(f"Done! Stock Results saved to {OUTPUT_DIR}")
