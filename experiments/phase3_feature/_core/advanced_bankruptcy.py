"""
Phase 3 — Study II: 進階非線性特徵選擇（破產）— 共用實作。
由 mutual_info / shap 子包各別呼叫，結果寫入 results/phase3_feature/<method>/。
"""
...
def _build_fs_configs(fs_method_names: List[str]) -> List[Tuple[str, Optional[str], Optional[float]]]:
    cfg: List[Tuple[str, Optional[str], Optional[float]]] = [("no_fs", None, None)]
    for method in fs_method_names:
        for ratio in FS_RATIOS:
            tag = f"{method}_r{int(ratio * 100)}"
            cfg.append((tag, method, ratio))
    return cfg
...
def _run_one_split_one_fs(
    ...
    X_val_all = pd.concat(
        [X_old_val_s, pre_old.scale_features(X_new_val, fit=False)[0]],
        ignore_index=True,
    )
...
def run_advanced(project_root: Path, results_method_subdir: str, fs_method_names: List[str]) -> None:
    ...
    logger.info(f"Study II: 進階非線性特徵選擇 × XGBoost 年份切割 — Bankruptcy ({results_method_subdir})")
    ...
    raw_path = output_dir / "xgb_bankruptcy_fs_advanced_raw.csv"
    ...
    summary_path = output_dir / "xgb_bankruptcy_fs_advanced_summary.csv"
    ...
    if not baseline.empty:
        ...
        if diff_rows:
            ...
            diff_path = output_dir / "xgb_bankruptcy_fs_advanced_auc_diff.csv"
    ...
    logger.info(f"\n=== Study II Advanced Bankruptcy 完成 ===")
