...
為美國三大指數 (S&P 500, Dow Jones, NASDAQ) 各跑一次實驗。
...
def run_experiment(dataset_name, logger):
    ...
    logger.info("  - Baseline 3: Ensemble (Old 3)")
...
def main():
    ...
    logger.info("=" * 80)
    logger.info("實驗 5: Stock Baseline + Ensemble (SPX, DJI, NDX)")
    logger.info("=" * 80)
    ...
    for ds in indices:
        try:
            ...
            # Add prefix to column names if we want to merge them, 
            # but simplest is saving separate CSVs for each index, and one average
            out_dir = project_root / "results/stock"
            out_dir.mkdir(parents=True, exist_ok=True)
            out_csv = out_dir / f"{ds}_baseline_ensemble_results.csv"
    ...
    # 計算三大指數的平均 AUC (為了相容原專欄輸出 stock_baseline_ensemble_results.csv)
    if all_results:
        avg_df = sum(all_results.values()) / len(all_results)
        out_csv = project_root / "results/stock/stock_baseline_ensemble_results.csv"
