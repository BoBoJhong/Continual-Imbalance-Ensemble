...
| DES_KNORAE | 0.8560 | 0.2224 | des |
| finetune | 0.8552 | 0.1936 | baseline |
| ensemble_4 | 0.8492 | 0.1951 | ensemble |
| ensemble_3_type_a | 0.8490 | 0.1851 | ensemble |
| ensemble_5 | 0.8402 | 0.1890 | ensemble |
| ensemble_3_type_b | 0.8384 | 0.1933 | ensemble |
| ensemble_2_old_hybrid_new_hybrid | 0.8373 | 0.1841 | ensemble |
| ensemble_old_3 | 0.8086 | 0.1327 | ensemble |
| retrain | 0.8047 | 0.1336 | baseline |

**分析**：  

- **ensemble_new_3**（僅用新模型）與 **DES**、**finetune** 皆明顯優於 **retrain**（AUC 約 0.869、0.856、0.855 vs. 0.805），顯示在持續學習與類別不平衡情境下，**適應策略優於單純合併重訓**。此結果支持本研究之核心論點：在 historical 與 new operating 具分布差異時，採用「新模型池」或「動態選擇」比單純合併重訓更有效。  
- 僅用舊模型（**ensemble_old_3**）與 **retrain** 接近（AUC 約 0.809 vs. 0.805），皆較差，符合「新時期需適應」之直覺——若僅依賴歷史模型或合併重訓而未凸顯新時期特性，表現可能不足。  
- **DES** 與 **ensemble_all_6**、**finetune** 接近（AUC 約 0.856、0.857、0.855），且 DES 之 F1（0.222）略優於 finetune（0.194），顯示動態選擇在少數類（破產）表現上具潛力，可作為實務建模之選項之一。
...
| stock | 0.5529 | **0.6134** | 0.5160 | 0.6043 | 0.5125 | 0.5143 |
| medical | 0.6621 | 0.6649 | 0.6509 | **0.6665** | 0.6660 | 0.6374 |

**說明**：  

- **Stock (S&P 500)**：股市崩盤預測（20天跌幅>5%）本身具極高難度，各方法 AUC 整體偏低（0.51～0.61）。其中，僅利用新資料微調 `finetune` 表現略優。結果顯示在具高度隨機性任務下，當新資料足夠對抗漂移時，單純微調也能成為有競爭力的基線。
...
**(1) 適應策略優於 Re-training**  
在破產資料上，**ensemble_new_3**、**DES**、**finetune** 一致優於 **retrain**（AUC 約 0.869、0.856、0.855 vs. 0.805），支持「在持續學習與類別不平衡下，採用新模型池或動態選擇比單純合併重訓更有效」之論點。此結果與文獻中「概念漂移下需適應新時期」之觀點一致（Lu et al., 2018），並可作為實務建模之參考：在歷史與新營運期具分布差異時，建議採用適應策略（新模型池、DES 或僅用新資料微調）而非單純合併重訓。

**(2) 進階 DES 之效益**  
時間加權與 combined 之小幅提升（AUC 約 +0.006–0.007）顯示，在 DES 中納入「新時期權重」或「少數類權重」具改進空間，可作為後續方法延伸之實證基礎；惟改進幅度有限，論文中可如實報告並討論可能原因（如參數設定、資料特性）。

**(3) 比例實驗之意涵**  
比例實驗結果支持「新資料佔比低時，適應策略相對 retrain 的優勢更大」之解讀：當新資料僅佔 20% 時，retrain 之 AUC 最低（0.811），而 ensemble_new_3 與 DES_combined 維持較高 AUC（0.869、0.859）。建議於論文中以「比例 vs. AUC」折線圖呈現，並撰寫為「何時適應策略領先 retrain」之實證支持。

**(4) 跨領域資料集（Stock 與 Medical）之解讀**  
在醫療資料（Medical）上，`ensemble_new_3` 維持一致的最佳表現，印證了適應策略在跨領域非平穩環境的普遍有效性。而在極高難度的股價崩盤預測（Stock）上，因市場本身存在難以預測性，整體預測表現偏低，且 `finetune` 略勝一籌。這說明了當新時期資料特徵發生劇烈且隨機的漂移時，直接對新資料進行 Fine-tuning 可能比建立複雜的 Ensemble 更敏捷有效。此結論可為後續研究依據不同難度任務選擇適當策略提供參考與發展方向。
