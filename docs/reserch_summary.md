...
#### 6.1.1 Bankruptcy（基準 AUC=0.8213，基於 XGBoost 年份切割靜態集成 All_6）

| 方法 | 保留比例 | AUC ↑ | F1 ↑ | Recall ↑ | AUC_Diff | 備註 |
|---|---|---|---|---|---|---|
| no_fs（對照）| 100% | 0.8213 | 0.1602 | 0.5329 | — | 基準線 |
| kbest_f | r80 | 0.8201 | 0.1598 | 0.5482 | -0.0012 | 線性方法性能退化 |
| **mutual_info** | **r80** | **0.8219 ★** | **0.1605 ★** | 0.5350 | **+0.0006** | 非線性 FS 成功突破 |
| shap_r80 | r80 | 0.8216 | 0.1601 | 0.5342 | +0.0003 | 模型權重導向 FS 有效 |
| **RFE (XGBoost)** | **r80** | **0.8506 ★** | 0.1571 | **0.6864 ★** | **+0.0293** | 封裝法取得顯著突破 |

**實驗洞察：**
1.  **非線性優勢**：在高度不平衡的財務數據中，線性方法（kbest_f）會誤刪關鍵特徵。改用互資訊 (MI) 與 SHAP 後，成功超越全特徵基準。
2.  **RFE 的爆發力**：遞迴特徵刪除 (RFE) 針對 XGBoost 量身定做，在資料充足的年份展現出最強的預測能力 (AUC 0.8506)， Recall 提升至 0.68，證明了模型驅動 FS 的上限更高。
3.  **動態集成穩定性**：特徵選擇有助於動態選擇器（如 KNORA-U）過濾掉干擾鄰域計算的特徵，提升了集成系統在概念漂移環境下的魯棒性。

#### 6.1.2 Medical（基準 AUC=0.6665）

| 特徵選擇方法 | 保留比例 | 靜態集成 (All_6) | 動態集成 (KNORA-U) | 備註 |
| :--- | :--- | :--- | :--- | :--- |
| **no_fs (Baseline)** | 100% | 0.6665 | 0.6511 | 基準線 |
| **mutual_info** | 80% | 0.6658 | **0.6532 ★** | 動態集成獲得顯著穩定提升 |

**實驗洞察：**
1.  **動態集成的魯棒性**：在醫療資料中，雖然特徵選擇對靜態集成的提升有限，但對**動態集成 (KNORA-U)** 卻能帶來正向影響（AUC 從 0.6511 升至 0.6532）。這顯示特徵選擇能優化局部競爭區域的界定。
2.  **Recall 的權衡**：在 `mutual_info` 下，醫療資料集的召回率 (Recall) 普遍變得更穩定，這在醫療診斷預測中具有高度的臨床價值。
